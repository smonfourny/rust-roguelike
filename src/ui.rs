...
use specs::prelude::*;
use super::{CombatStats, GameLog, Player, BASE_BG_COLOR, CYAN_COLOR, HEALTH_OFFSET, HEALTHBAR_OFFSET, LOG_OFFSET, MAP_X, MAP_Y, ORANGE_COLOR, RED_COLOR, WHITE_COLOR};
...
pub fn draw_ui(ecs: &World, ctx: &mut BTerm) {
    ...
    for (_player, stats) in (&players, &combat_stats).join() {
        ...
        ctx.print_color(HEALTH_OFFSET, MAP_Y, RGB::named(health_color), RGB::named(BASE_BG_COLOR), &hp_message);

        ctx.draw_bar_horizontal(HEALTHBAR_OFFSET, MAP_Y, HEALTHBAR_OFFSET+20, stats.hp, stats.max_hp, RGB::named(health_color), RGB::named(BASE_BG_COLOR));
    }
